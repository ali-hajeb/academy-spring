    ...
    searchCourse: (state) => {
      if (state.searchClue) {
        state.coursesList = state.rawCoursesList.filter((course) => {
          const doseContainCourseTitle = course.title
            .toLowerCase()
            .includes(state.searchClue.toLowerCase());
          const doseContainCourseTeacher = course.teacher.fullName
            .toLowerCase()
            .includes(state.searchClue.toLowerCase());
          const doseContainCourseLessonName = course.lesson.lessonName
            .toLowerCase()
            .includes(state.searchClue.toLowerCase());
          const doseContainCourseTopics = course.lesson.topics.includes(
            state.searchClue.toLowerCase()
          );
          return (
            doseContainCourseTeacher ||
            doseContainCourseTitle ||
            doseContainCourseTopics ||
            doseContainCourseLessonName
          );
        });
      } else {
        state.coursesList = state.rawCoursesList;
  ...
  extraReducers: {
    ...
    [coursesAction.getCourses.fulfilled.toString()]: (state, action) => {
      ...
      if (action.meta.arg.pageSize === 'ALL') {
        ...
        state.paginationPagesCount = Math.ceil(
          payload.result.count / action.meta.arg.pageSize
        );
      }
    },
