...
if (res.data.message[0].message === "شما قبلا این دوره را لایک کردید"){
  try {
    
    if (countRes.data.success) {
     
     setCourse({...course, count: +like - +dislike, status:'succeeded', payload:data.result, isStudentEnrolled:!!isStudentEnrolled, isLiked:true})
   }
   } catch (error) {
     console.log(error);
  ...
  if (res.data.message[0].message !== "شما قبلا این دوره را دیسلایک کردید") {
  try {
  
   if (countRes.data.success) {
    
    setCourse({...course, count: +like - +dislike, status:'succeeded', payload:data.result, isStudentEnrolled:!!isStudentEnrolled, isLiked:false})
  }
  } catch (error) {
    console.log(error);
